...
with open(os.path.join(RESULTS_DIR, "README_results.txt"), "w") as f:
    f.write(
        "Results of MobileNetV2 10-fold CV on the data-ar dataset (arabica vs robusta)\n"
        f"Classes: {CLASS_NAMES}\n"
        "Per-fold files: {1..10}_history.csv, {1..10}_training_plot.png, "
        "{1..10}_confusion_matrix.png, {1..10}_roc.png, {1..10}_classification_report.txt, {1..10}_meta.json\n"
        "Aggregate files: cv_metrics_per_fold.csv, cv_metrics_summary.csv\n"
    )

print("✅ 10-fold cross-validation completed. Results saved in folder:", RESULTS_DIR)
