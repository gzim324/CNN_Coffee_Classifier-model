...
EPOCHS_PHASE1 = 5
EPOCHS_PHASE2 = 20
RESULTS_DIR = "results_resnet101_ft"
DATA_DIR = "data-ar"
N_SPLITS = 10
SEED = 42
...
def make_dataset(paths, y, training=False):
    ...
    ds = ds.map(lambda p, t: (decode_and_preprocess(p), t), num_parallel_calls=AUTOTUNE)
    if training:
...
def unfreeze_for_finetune(model, base_model, last_trainable=LAST_TRAINABLE, keep_bn_frozen=True):
    base_model.trainable = True
    start = max(0, len(base_model.layers) - int(last_trainable))
...
with open(cv_all_csv, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow([
        "fold", "val_accuracy", "precision", "recall", "f1", "roc_auc",
        "phase1_best_epoch", "phase2_best_epoch", "train_time_s"
    ])
...
for fold_idx, (train_idx, val_idx) in enumerate(skf.split(filepaths, labels), start=1):
    ...
    hist_p1 = model.fit(
        ...
        epochs=EPOCHS_PHASE1, callbacks=[early_stopping_p1], verbose=1
    )
    plot_training(hist_p1, prefix=f"{prefix}_phase1")
    save_history_csv(hist_p1, prefix=f"{prefix}_phase1")

    unfreeze_for_finetune(model, base_model, last_trainable=LAST_TRAINABLE, keep_bn_frozen=True)
    hist_p2 = model.fit(
        train_ds, validation_data=val_ds,
        epochs=EPOCHS_PHASE2, callbacks=[early_stopping_p2], verbose=1
    )
    train_time = time.time() - t0
    plot_training(hist_p2, prefix=f"{prefix}_phase2")
    save_history_csv(hist_p2, prefix=f"{prefix}_phase2")
    ...
    with open(os.path.join(RESULTS_DIR, f"{prefix}_classification_report.txt"), "w") as f:
        f.write(
            ...
            f"phase1_best_epoch(val_auc): {int(np.argmax(hist_p1.history.get('val_auc', [0])) + 1)}\n"
            f"phase2_best_epoch(val_auc): {int(np.argmax(hist_p2.history.get('val_auc', [0])) + 1)}\n"
        )

    with open(cv_all_csv, "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            fold_idx, f"{acc:.6f}", f"{prec:.6f}", f"{rec:.6f}",
            f"{f1:.6f}", f"{auc_val:.6f}",
            int(np.argmax(hist_p1.history.get("val_auc", [0])) + 1),
            int(np.argmax(hist_p2.history.get("val_auc", [0])) + 1),
            f"{train_time:.3f}"
        ])
...
with open(os.path.join(RESULTS_DIR, "README_results.txt"), "w") as f:
    f.write(
        "Results of ResNet101 (ImageNet fine-tune) 10-fold CV on the data-ar dataset (arabica vs robusta)\n"
        f"Classes: {class_names}\n"
        "Per fold: {1..10}_phase1_training_plot.png, {1..10}_phase1_history.csv, "
        "{1..10}_phase2_training_plot.png, {1..10}_phase2_history.csv, "
...
print(f"\n✅ 10-fold cross-validation with fine-tuning completed. Results: {RESULTS_DIR}")
